...
- **笔记与知识库**：新增 `/notes`、`/admin/notes` 和 `/admin/notebooks`，支持随记、主题笔记、笔记本编排、标签搜索、公开状态与知识库收录状态筛选。
- **笔记版本工作流**：工作稿、公开版和知识库收录版使用独立快照；支持自动保存、历史版本恢复、乐观锁、发布/撤回、回收站恢复以及笔记本归档/恢复。
- **MCP 笔记工具**：新增 `search_knowledge`、`get_note`、`list_notebooks`、`get_notebook`、`create_note`、`update_note`；笔记读取和写入使用独立授权，可限制到指定笔记本，AI 写入只生成工作稿，不会自动发布或收录。
- **笔记私密图片**：新增受保护的 `/note-media/<uuid>` 存储与鉴权，单图限制 5 MiB，并保护历史版本、回收站和素材引用不被误删。

### Changed

- **编辑体验**：快速记录与笔记编辑器增加标签页恢复副本、未保存离开保护和保存期间输入保护。
- **MCP 令牌管理**：增加笔记授权表单的必填校验、错误提示和失败后自动聚焦。

### Fixed

- 统一笔记筛选器、笔记本与编辑器选择控件及编辑页侧栏样式。
- 补齐 `rua` 部署所需的代码运行器镜像拉取与完整性校验；镜像缺失时阻止应用重启。
...
- **全站视图过渡与双向状态恢复**：引入原生 View Transitions API，实现前台跨页面平滑视图过渡；顶栏导航支持栏目平移切换动效与下划线共享过渡；实现文章列表与文章详情页的双向滚动位置与浏览状态精确恢复，隔离主题切换动画并协调过渡中断。
- **前台公共页面重构**：
  - **数字花园首页**：全新设计首页文章流与卡片层次、列表悬停与进入动效，重设计错误态与重试交互反馈。
  - **归档时间线与标签合并**：重设计归档时间线与响应式阅读层次；顶部新增默认折叠的标签索引，支持平滑折叠、标签错峰显隐与悬停反馈；旧 `/tags` 地址永久重定向至 `/archives`。
  - **搜索索引与交互**：重设计文字索引展示，支持防抖自动搜索、中文输入法保护与实时高亮。
  - **花园刊物专题**：重设计标签刊物专题页面（`/tags/:slug`）及响应式排版与交互动效。
  - **友链名片与世界树关于页**：重设计友链名片悬停反馈与头像加载动效，重构关于页设计与排版。
- **前端库按需加载**：CodeMirror 编辑器、Tiptap 富文本、Mermaid 流程图、xterm 终端等重量级前端库按需动态加载，重构 Mermaid 共享渲染器加载与竞态处理，显著优化首屏包体积与资源加载策略。
- **构建与环境增强**：
  - 在 `make` 与 `make dev` 前增加 CLI 工具依赖检查与 `llvm-tools` 组件检查。
  - Docker 支持 `build-runners` 镜像构建参数。
...
- **架构与构建精简**：移除 FreeBSD 交叉编译配置与构建目标，精简仓库文档与冗余上下文文件。
- **设置页与运行器重构**：复用回收站配置的数据库读写，整理冗余常量与代码运行器配置解析。

### Fixed

- **UI 与交互修复**：
  - 统一全站文字选区为主题绿色。
  - 修复首屏主题图标与保存模式的同步逻辑，修复三态循环与刷新后的主题图标状态。
  - 统一头像与文章封面占位回退，为加载失败的封面添加占位样式，修复客户端导航后的封面高度与灯箱点击。
  - 恢复图片灯箱滚动缩回原图的过渡动画。
  - 修复 SPA 切换文章后目录高亮失效的问题。
  - 修复后台文章独立滚动区域在导航后的位置恢复，统一文章预览入口以保留返回状态。
- **编辑器与构建修复**：
...
- **首页阅读版式**：首页及后续分页共用侧栏与文章流，移动端分页使用紧凑页头，分页导航显示当前页与总页数。移除呼吸徽标和重复导航胶囊；所有文章统一使用轻量列表和可选缩略图，不再单独强调最新一篇。同步深浅主题、键盘焦点与加载占位，保留现有搜索、标签和分页路由。
...
- **终端尺寸自适应**：代码运行器容器尺寸变化时自动重新计算终端列宽，适配侧栏切换、窗口调整和输出面板展开。

### Changed

- **后台组件与骨架屏收敛**：导航、分页、表单、文章/SQL 表格和骨架屏统一复用共享原子，并补齐列表稳定 key，减少重复实现。
- **启动与部署链路加固**：生产监听改为直接绑定，强化启动引导与路由生命周期；部署时强制重建应用镜像。
- **WASM 质量门禁增强**：为 web-only 目标补充 Clippy 检查。

### Fixed

- 修复 CodeMirror 中 Go 注释高亮错误。
- 修复可运行代码块编辑弹窗语言下拉触发器未挂载、控件样式与主题不一致的问题。
- 修复列表项内代码块与正文间距过小。
- 修复后台侧栏子菜单在组内分页时被强制重新展开，以及菜单 effect 自触发导致页面卡死的问题。
- 修复评论作者网站链接无文字、回复表单切换时草稿丢失的问题。
- 修复素材单图上传绕过文件大小/MIME 校验、关闭按钮缺少键盘焦点样式及弹窗语义不完整的问题。

### Security

- 写请求在无法确定本站 origin 或来源头缺失时改为拒绝，修复 CSRF fail-open。
- 修复 IPv4-mapped IPv6 地址绕过 URL 抓取内网地址拦截的问题。
- 会话缓存命中时重新校验会话存在性、过期时间、管理员角色和 session generation；限制密码最大长度为 256 字节。
- 备份导入不再把 `DATABASE_URL` 传入 `pg_dump`/`psql` 参数，拒绝非允许的 psql 元命令，并在恢复后清理全部相关缓存。
- 修复素材删除与孤儿清理的并发窗口，避免新增引用被误删。
...
- **素材引用明细浮层**：素材管理支持查看文章、评论和头像的全部引用来源。

### Fixed

- 修复素材引用统计未计入头像图片。
- **CI 双机并行部署**：xun 与 rua 拆为独立 job，在 GHCR 镜像发布后并行拉取并滚动重启；runner 镜像改用公开标签，避免独立私有包无法匿名拉取。
- **CI runner 镜像完整性门禁**：发布任务核对 GHCR 的 6 个 runner tag，部署前核对 xun 本地最终 tag，缺失时阻止 app 重启，避免代码运行器返回 404。

## [0.12.0] - 2026-08-19

### Added

- **登录用户免表单直发评论**：文章评论区在登录态下切换为「身份行（头像 + 显示名）+ 内容框」，免去昵称/邮箱/网站字段与蜜罐，评论直发 approved 免审核，名字旁显示「作者」徽章。`comments` 表新增 `user_id` 外键（迁移 024），读取时 LEFT JOIN users 实时解析显示名与头像；改名或换头像后历史评论自动跟随。
- **后台个人信息与头像**：新增 `/admin/profile` 个人资料页，支持显示名称、邮箱、头像素材选择、移除头像与修改密码；密码修改会使其他设备会话失效，侧栏新增用户身份卡片，并统一复用 `UserAvatar` 组件。
- **评论富文本与图片上传**：评论表单接入精简版 Tiptap 所见即所得编辑器，支持按钮上传和粘贴图片；新增评论图片上传端点、匿名双层限流与评论 HTML 图片标签放行。
- **备份导入恢复**：新增备份导入接口，支持 multipart 流式落盘、签名校验、同名冲突拒绝、磁盘空间预检、临时文件原子入库与 `BACKUP_IMPORT_MAX_MB` 单文件上限。
- **评论引用感知的孤儿素材清理**：新增孤儿素材定期清理任务，识别评论中的素材引用并按保留期回收未引用上传文件。
- **后台页面体验重构**：重做文章、回收站、评论、写作、素材、友链、MCP、代码试运行、系统与设置等管理页面，统一卡片化布局、导航、操作反馈和诊断信息展示。

### Changed

- **编辑器与界面组件统一**：增加 Tiptap `full` / `comment` 变体机制，统一文章编辑器与后台/前台 Checkbox 样式，收敛公共表单、Tooltip、Popover 等组件。
- **后台交互细节收紧**：评论和素材相关操作采用一致的上传错误清理、过渡动画、表格滚动与按钮对齐行为。

### Fixed

- 修复回收站和评论管理表格出现双滚动条的问题。
- 修复 Tiptap `full` 变体因浅合并 `editorProps` 覆盖默认配置而挂载崩溃。
- 修复取消代码运行后容器清理不完整的问题。
- 修复后台骨架屏重复页头造成的高度溢出与滚动条抖动，以及 Tooltip 的 `group` 命名空间冲突。
- 修复从后台切回前台后文章列表点击失效、评论提交按钮尺寸对齐异常等问题。
- 修复跨环境备份恢复的数据库属主兼容性与失败回滚，确保任一 SQL 失败时不留下半恢复数据库。

### Security

- 评论图片上传增加匿名分层限流，降低滥用风险。
