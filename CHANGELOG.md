...
- **登录用户免表单直发评论**：文章评论区在登录态下切换为「身份行（头像 + 显示名）+ 内容框」，免去昵称/邮箱/网站字段与蜜罐，评论直发 approved 免审核，名字旁显示「作者」徽章。`comments` 表新增 `user_id` 外键（迁移 024），读取时 LEFT JOIN users 实时解析显示名与头像——改名/换头像后历史评论自动跟随（保存资料时失效全部评论缓存）；匿名评论的填写/待审/轮询流程不变。
- **共享 `UserAvatar` 组件**：头像「图片 / 首字符」两态的统一实现，收敛后台侧栏用户卡片、个人信息页身份卡、前台评论表单身份行三处调用。
