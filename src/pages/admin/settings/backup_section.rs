...
pub fn BackupSection(toast: Callback<(String, bool)>) -> Element {
            ...
            div { class: "space-y-6",
                div { class: "{ADMIN_CARD_CLASS} p-6 md:p-8 flex flex-col gap-6",
                    ...
                    div { class: "flex flex-col gap-2 max-w-xl",
                        FormLabel {
                            label: "执行时间",
                            html_for: Some("backup-time".to_string()),
                        }
                    ...
                    div { class: "flex flex-col gap-2 max-w-xl",
                        FormLabel {
                            label: "自动备份保留份数",
                            html_for: Some("backup-retention".to_string()),
                        }
                        input {
                            id: "backup-retention",
                            r#type: "number",
                            ...
                            oninput: move |e: Event<FormData>| {
                                ...
                                if let Ok(n) = v.parse::<i32>() {
                                    retention_draft.set(n);
                                }
                                just_saved.set(false);
                            },
                        }
                        p { class: "text-xs text-[var(--color-paper-secondary)]",
                            "超出后最旧的备份连配对 uploads 包一起删除（1–365）。手动备份永不自动删除。"
                        }
                    }
    ...
    {
        rsx! {
            div { class: "p-8 text-[var(--color-paper-secondary)]", "备份配置（前端渲染）" }
        }
    }
}
