...
pub fn RunnerSection(toast: Callback<(String, bool)>) -> Element {
        ...
        rsx! {
            div { class: "space-y-6",
                div { class: "{ADMIN_CARD_CLASS} p-6 md:p-8 flex flex-col gap-6",
                    ...
                    label { class: "flex items-center gap-3 cursor-pointer max-w-xl",
                        ...
                        div { class: "flex flex-col",
                            ...
                            span { class: "text-xs text-[var(--color-paper-secondary)]",
                                "⚠️ 启用后用户代码可访问网络，存在安全风险。"
                            }
                        }
                    }

                    NumberField {
                        id: "runner-concurrent".to_string(),
                        label: "最大并发任务数".to_string(),
                        value: draft().max_concurrent.to_string(),
                    ...
                    NumberField {
                        id: "runner-cpu".to_string(),
                        label: "每任务最大 CPU 核数".to_string(),
                        value: format!("{:.1}", draft().max_cpu_cores),
                        oninput: move |v: String| {
                    ...
                    NumberField {
                        id: "runner-mem".to_string(),
                        label: "每任务最大内存（MB）".to_string(),
                        value: draft().max_memory_mb.to_string(),
                        oninput: move |v: String| {
                            if let Ok(n) = v.parse::<u32>() {
                                let mut d = draft();
                                d.max_memory_mb = n;
                    ...
                    NumberField {
                        ...
                        label: "每任务最大执行超时（秒）".to_string(),
                        value: draft().max_timeout_secs.to_string(),
                        oninput: move |v: String| {
                            if let Ok(n) = v.parse::<u32>() {
                                let mut d = draft();
                                d.max_timeout_secs = n;
                                draft.set(d);
                                just_saved.set(false);
                            }
                        },
                    ...
                    NumberField {
                        ...
                        label: "每任务最大输出字节数".to_string(),
                        value: draft().max_output_bytes.to_string(),
                        oninput: move |v: String| {
                            if let Ok(n) = v.parse::<u64>() {
                                let mut d = draft();
                                d.max_output_bytes = n;
                                draft.set(d);
                                just_saved.set(false);
                            }
                        },
                    ...
                    NumberField {
                        id: "runner-source".to_string(),
                        label: "每任务最大源码字节数".to_string(),
                        value: draft().max_source_bytes.to_string(),
                        oninput: move |v: String| {
                            if let Ok(n) = v.parse::<u64>() {
                                let mut d = draft();
                                d.max_source_bytes = n;
                                draft.set(d);
                                just_saved.set(false);
                            }
                        },
                    ...
                    NumberField {
                        id: "runner-queue".to_string(),
                        label: "排队等待超时（秒）".to_string(),
                        value: draft().queue_timeout_secs.to_string(),
                        oninput: move |v: String| {
                            if let Ok(n) = v.parse::<u32>() {
                                let mut d = draft();
                                d.queue_timeout_secs = n;
                                draft.set(d);
                                just_saved.set(false);
                            }
                        },
                    ...
                    NumberField {
                        id: "runner-ttl".to_string(),
                        label: "历史 task 保留时长（秒）".to_string(),
                        value: draft().task_ttl_secs.to_string(),
                        oninput: move |v: String| {
                            if let Ok(n) = v.parse::<u32>() {
                                let mut d = draft();
                                d.task_ttl_secs = n;
                    ...
                    div { class: "flex flex-col gap-2 max-w-xl",
                        FormLabel {
                            label: "语言白名单（逗号分隔，留空=全部）",
                            html_for: Some("runner-langs".to_string()),
                        }
                        input {
                            id: "runner-langs",
                            r#type: "text",
    ...
    {
        rsx! {
            div { class: "p-8 text-[var(--color-paper-secondary)]", "运行器配置（前端渲染）" }
        }
    }
}
#[cfg(target_arch = "wasm32")]
