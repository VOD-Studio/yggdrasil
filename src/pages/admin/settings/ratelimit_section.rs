...
pub fn RateLimitSection(toast: Callback<(String, bool)>) -> Element {
        ...
        rsx! {
            div { class: "space-y-6",
                div { class: "{ADMIN_CARD_CLASS} p-6 md:p-8 flex flex-col gap-6",
                    ...
                    LimiterField {
                        label: "严格限流（登录/注册）",
                        per_sec: draft().strict_per_sec,
                        burst: draft().strict_burst,
                        on_per_sec: move |v| {
                            let mut d = draft();
                            d.strict_per_sec = v;
                            draft.set(d);
                    ...
                    LimiterField {
                        label: "上传限流（图片上传）",
                        per_sec: draft().upload_per_sec,
                        burst: draft().upload_burst,
                        on_per_sec: move |v| {
                            let mut d = draft();
                            d.upload_per_sec = v;
                            draft.set(d);
                    ...
                    LimiterField {
                        label: "图片访问限流（GET /uploads/*）",
                        per_sec: draft().image_per_sec,
                        burst: draft().image_burst,
                        on_per_sec: move |v| {
                            let mut d = draft();
                            d.image_per_sec = v;
                            draft.set(d);
                    ...
                    LimiterField {
                        label: "评论限流（创建评论）",
                        per_sec: draft().comment_per_sec,
                        burst: draft().comment_burst,
                        on_per_sec: move |v| {
                            let mut d = draft();
                            d.comment_per_sec = v;
                            draft.set(d);
                    ...
                    LimiterField {
                        label: "unknown 桶（无法识别 IP）",
                        per_sec: draft().unknown_per_sec,
                        burst: draft().unknown_burst,
                        on_per_sec: move |v| {
                            let mut d = draft();
                            d.unknown_per_sec = v;
                            draft.set(d);
                    ...
                    div { class: "flex flex-col gap-2 max-w-xl",
                        FormLabel {
                            label: "代码执行日限额（次/天）",
                            html_for: Some("rl-code-daily".to_string()),
                        }
                        input {
                            id: "rl-code-daily",
                            r#type: "number",
                    ...
                    div { class: "flex items-center justify-between gap-4 pt-1",
                        ...
                        LoadingButton {
                            ...
                            onclick: move |_| {
                                ...
                                saving.set(true);
                                spawn(async move {
                                    match update_rate_limit_settings(
                                            d.strict_per_sec,
                                            d.strict_burst,
                                            d.upload_per_sec,
                                            d.upload_burst,
    ...
    {
        rsx! {
            div { class: "p-8 text-[var(--color-paper-secondary)]", "限流配置（前端渲染）" }
        }
    }
}
#[cfg(target_arch = "wasm32")]
        ...
        div { class: "flex flex-col gap-2 max-w-xl",
            ...
            div { class: "flex gap-3",
                div { class: "flex-1",
                    ...
                    p { class: "text-xs text-[var(--color-paper-tertiary)] mt-1",
                        "每秒请求数"
                    }
                }
                div { class: "flex-1",
