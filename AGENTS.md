...
- `src/` — Rust source (single crate). `main.rs` (entry), `router.rs` (Dioxus routes), `middleware.rs` (axum layers: compression, cache-control, admin_guard), `cache.rs` (moka, many domain caches with distinct TTLs), `ssr_cache.rs` (physical SSR cache invalidation), `theme.rs` (light/dark/system), `highlight.rs` (syntect), `context.rs` (`UserContext` global login state), `config.rs` (runtime-baked LazyLock config singletons), `build_info.rs` (build.rs-injected git/rustc metadata).
  ...
  - `src/models/` — `post.rs`, `user.rs`, `comment.rs`, `settings.rs`, `asset.rs`, `friend_link.rs`, `mcp_token.rs`, `system.rs` (`SystemSnapshot`) (serde DTOs shared across SSR/cache/API).
  ...
  - `src/tasks/` — server-only background loops spawned in `serve()`: `session_cleanup.rs`, `post_purge.rs`, `image_cache_cleanup.rs`, `ip_purge.rs`, `orphan_asset_purge.rs` (unreferenced-uploads purge), `backup.rs` (scheduled backup), `sysinfo_sampler.rs` (system metrics sampler; the `SystemSnapshot` DTO lives in `models/system.rs`).
  - `src/infra/` — `docker.rs` (bollard), `runner_config.rs`, `webp.rs` (zenwebp encode/decode wrapper).
  ...
  - `src/bridges/` — wasm-bindgen bridges for JS editors/terminal (`tiptap.rs`, `codemirror.rs`, `xterm.rs`).
- `libs/` — pnpm JS workspace, packages named `@yggdrasil/*`. Each builds to a self-contained IIFE bundle written **directly into `public/<dir>/`** and consumed by the Rust side via window globals (`js_sys::Reflect::get` on object-literal modules, or global `__init*` functions via a typed `invoke_optional_global` helper).
  - `tiptap-editor` → `public/tiptap/` (rich-text Markdown editor with two variants: `full` for admin posts, `comment` — a subset with bubble menu and no headings/tables/slash commands — for the frontend comment form), `codemirror-editor` → `public/codemirror/` (code-runner source editor), `lightbox` → `public/lightbox/`, `xterm-terminal` → `public/xterm/`, `yggdrasil-core` → `public/yggdrasil-core/`, `mermaid-renderer` (dynamically script-injected by yggdrasil-core on viewport visibility), `shared` (cross-lib constants: `ThemeName`, `THEME_CHANGE_EVENT` — inlined into each IIFE, not bundled).
- `migrations/` — 26 numbered SQL files (`NNN_desc.sql`); each must also be registered in the `MIGRATIONS` array in `src/db/migrate.rs` (enforced by the `migrations_match_files_on_disk` test).
...
**Self-contained binary**: migrations (`src/db/migrate.rs` `include_str!`), custom syntaxes (`src/highlight.rs` `include_str!`), and `public/highlight.css` (pre-generated at build time) are embedded — the runtime image (`alpine:3.22` + `ca-certificates` for reqwest's Rustls/AWS-LC platform trust + `postgresql16-client` for `pg_dump`/`psql` backup support) needs only the binary + `public/` + `uploads/`.

**KaTeX upgrade rollout (migration 026)**: `katex-rs` 0.3 uses the npm KaTeX 0.18.x stylesheet shipped by `make katex-css`. Migration 026 renames layout class tokens only inside persisted KaTeX HTML in all posts/comments, including drafts and trash, in one transaction; source content and timestamps are preserved. Each instance clears public/preview SSR caches after database bootstrap. Back up the database and **stop all old-version instances before starting the first upgraded instance**: old renderers must not write legacy class names after migration. A rollback must restore matching stored formula HTML and static assets, not just the old binary.
...
13. **WebP**: the `image` crate's `"webp"` feature is **intentionally excluded** — all WebP encode/decode goes through zenwebp (`src/infra/webp.rs`). Do NOT add it.
...
- **Transport**: official `rmcp` crate on stable **`3.2`**, with the resolved release recorded in `Cargo.lock`; no exact prerelease pin. Mounted via `StreamableHttpService` → `Router::nest_service("/mcp", service)` in `src/mcp/router.rs`, merged into the app router in `src/main.rs`. rmcp retains Origin/Host validation, `MCP-Protocol-Version` 400, the 4MiB body cap, and stateless transport configuration.
...
GitHub Actions (`.github/workflows/ci.yml`), **十一** jobs following the 2026 native multi-runner pattern (zero QEMU): three run on **every push, PR, and manual trigger** — `test` (cargo fmt/clippy + `cargo test --all-features`), `lint-js` (Biome on `libs/`), and `check-wasm` (wasm32 target check). `build-amd64` (native x86_64 runner, **plain `Dockerfile`** at repo root with cargo-chef + GHA layer cache; calls `docker buildx` directly — NOT `make docker-amd64` — to inject `--cache-from/to type=gha`; includes a container-boot smoke test that runs the image without Postgres and greps the startup banner) runs on **`main`/`master` push, tag push, or manual** (PRs and non-trunk branches rely on `test` only — saving CI minutes and GHA cache quota). `build-arm64` (native `ubuntu-24.04-arm` runner, plain Dockerfile builds `aarch64-unknown-linux-musl` natively — 4-8× faster than QEMU), `publish-ghcr` (merges amd64+arm64 into a multi-arch manifest → public GHCR package; deploy jobs pull `latest` from here), `publish-runners` (builds the 6 Code Runner sandbox images → `runner-*` tags in the same public GHCR package), `release` (three GitHub Release assets: `yggdrasil-amd64.tar.gz`, `yggdrasil-arm64.tar.gz`, `yggdrasil-x86_64-musl.tar.gz`; body extracted from `CHANGELOG.md`), `deploy-xun` (SSH to xun → pulls app + 6 runner images from GHCR with 3× retry, tags runners to the `LANGUAGES`-registry short names `yggdrasil-runner-*:latest` → `docker compose up -d app` rolling restart, postgres/data volumes untouched → polls `/readyz`; runner pulls are best-effort — a failed pull keeps the existing tag), and optional `deploy-rua` (SSH to rua → pulls the GHCR app manifest with Podman, runs `podman compose up -d app` through `pg-tunnel` to xun PG, then polls `/readyz`; enabled by repo variable `RUA_DEPLOY_ENABLED=true`; xun and rua are independent jobs and deploy in parallel; rua remains best-effort) all run **only on tag push (`v*`) or `workflow_dispatch`**. **Note**: CI builds with the plain `Dockerfile` (repo root) natively on each arch's runner; `Dockerfile.cross` (repo root, two-builder glibc+zig cross-compile) is the **local-only** path used by `make docker-amd64` / `scripts/xun.fish` on Apple Silicon — CI never invokes it. Required `deploy-xun` secrets: `XUN_SSH_KEY`, `XUN_HOST`, `XUN_USER`; optional `XUN_PORT`, `XUN_DEPLOY_DIR` (default `/root/docker/yggdrasil`), `XUN_KNOWN_HOSTS` (server fingerprint — recommended over the `ssh-keyscan` TOFU fallback). Optional `deploy-rua` secrets when `RUA_DEPLOY_ENABLED=true`: `RUA_SSH_KEY`, `RUA_HOST`, `RUA_USER`; optional `RUA_PORT` (default 29888), `RUA_DEPLOY_DIR` (default `/root/docker/yggdrasil`), `RUA_KNOWN_HOSTS`.
## Agent skills
